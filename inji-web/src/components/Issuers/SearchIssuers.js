...
import {getSearchIssuersUrl} from '../../utils/config';
...
const getSelectedIssuerObject = (formattedOptions, name) => {
    let issuerDetails = formattedOptions.filter(
...
function SearchIssuers() {
    ...
    function getReqValue(value) {
        if (value) {
            ...
            navigate(`/issuers/${reqValue?.value}`, {
                state: {
                    issuerDisplayName: reqValue?.title,
    ...
    function setFilterOptions(event) {
        ...
        _axios
            .get(getSearchIssuersUrl(value), {
                signal: abortController.signal,
            })
            .then(response => {
                if (response?.data?.response?.issuers) {
                    setFormatedOptions(
                        response?.data?.response?.issuers
                            .map(option => {
                                return {
                            ...
                            .filter(option =>
                                removeUinAndESignetIssuers(option.label),
                            ),
                    );
                    setLoadingIssuers(false);
                }
    ...
    function onClickRedirect(event) {
        ...
        if (value && value?.length > 0) {
            const issuerDetails = getSelectedIssuerObject(
                formatedOptions,
                value,
            );
            navigate(`/issuers/${value}`, {
                state: {
                    issuerDisplayName: issuerDetails?.title,
                    clientId: issuerDetails?.clientId,
                },
            });
        }
    }
            ...
            <StyledGrid container>
                ...
                <Grid item xs={12}>
                    <Box
                        ...
                        <PageSubTitle>
                            Please search for the issuer and in the next step,
                            select credential to download.
                        </PageSubTitle>
                    </Box>
                </Grid>
