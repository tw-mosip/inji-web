...
function Issuer() {
    ...
    const [issuerClientId, setIssuerClientId] = useState(
        location.state ? location.state['clientId'] : undefined,
    );
    ...
    useEffect(() => {
        _axios
            .get(getCredentialsSupportedUrl(issuerId))
            .then(response => {
                setCredentialsList(
                    response?.data?.response?.supportedCredentials,
                );
                setDefaultList(response?.data?.response?.supportedCredentials);
                setAuthEndpoint(
                    response?.data?.response?.authorization_endpoint,
                );
                setLoading(false);
            })
            .catch(error => {
    ...
    useEffect(() => {
        ...
        _axios
            .get(getSearchIssuersUrl(issuerDisplayName))
            .then(response => {
            ...
            .catch(error => {
                console.error(
                    'Failed to search for the issuer',
                    issuerId,
                    error,
                );
    ...
    return (
        <PageTemplate>
            <Header
                issuerDisplayName={
                    issuerDisplayName ? issuerDisplayName : issuerId
                }
                credentialsList={credentialsList}
                loading={loading}
                updateCredentialsList={setCredentialsList}
