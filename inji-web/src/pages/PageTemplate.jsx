...
function PageTemplate({children}) {
    ...
    useEffect(() => {
        ...
        if (Object.keys(searchParamsMap).indexOf('code') !== -1) {
            ...
            if (
                !sessionState ||
                !vcRedirectionDetails ||
                sessionState !== vcRedirectionDetails['state']
            ) {
            ...
            if (vcRedirectionDetails) {
                navigate(
                    `issuers/${vcRedirectionDetails.issuerId}/certificate/${vcRedirectionDetails.certificateId}` +
                    location.search,
                    {
                        state: {
                            issuerDisplayName:
                            vcRedirectionDetails.issuerDisplayName,
                        },
                    },
                );
            }
