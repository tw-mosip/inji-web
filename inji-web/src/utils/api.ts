...
export class api {
    ...
    static fetchRequest = async (uri: string, method: MethodType, body?: any, responseType?: string) => {
    ...
    static invokeDownloadCredential = async (issuerId: string, certificateId: string, token: string) => {
        let response;
        try {
            response = await api.fetchRequest(api.vcDownload(issuerId, certificateId), "GET", {
                method: "GET",
                headers: {
                    'Bearer': token,
